...
class Properties extends CI_Controller
{
    ...
    public function ViewContract($templateName = 'sale_contract2')
    {
    ...
    public function filter_search()
    {
        ...
        if ($MinPrice = $this->input->post('txtMinPrice'))
            $this->db->where('tbl_properties.TotalPrice >=', $MinPrice);
        if ($MaxPrice = $this->input->post('txtMaxPrice'))
            $this->db->where('tbl_properties.TotalPrice <=', $MaxPrice);
        if ($MinArea = $this->input->post('txtMinArea'))
            $this->db->where('tbl_properties.CoveredArea >=', $MinArea);
        if ($MaxArea = $this->input->post('txtMaxArea'))
            $this->db->where('tbl_properties.CoveredArea <=', $MaxArea);
    ...
    public function SavePropertyInformation($Case = 'Add', $PropertyId = 0)
    {
        ...
        $ModelDataProperty = [
            ...
            'ZipCode' => trim((string) $this->input->post('numZipCode')) !== '' ? trim($this->input->post('numZipCode')) : null,
            'Suburb' => trim((string) $this->input->post('suburb')) !== '' ? trim($this->input->post('suburb')) : null,
            'State' => trim((string) $this->input->post('state')) !== '' ? trim($this->input->post('state')) : null,
            'UnitNumber' => trim((string) $this->input->post('txtUnitNumber')) !== '' ? trim($this->input->post('txtUnitNumber')) : null,
            'BuildingNumber' => trim((string) $this->input->post('txtBuildingNumber')) !== '' ? trim($this->input->post('txtBuildingNumber')) : null,
            'StreetNumber' => trim((string) $this->input->post('txtStreetNumber')) !== '' ? trim($this->input->post('txtStreetNumber')) : null,
            'StreetName' => trim((string) $this->input->post('txtStreetName')) !== '' ? trim($this->input->post('txtStreetName')) : null,
    ...
    public function AddDocuments($Case = 'Add', $PropertyId = 0, $StationId = 0)
    {
        ...
        if (!is_array($DocTypes) && !is_object($DocTypes))
            $DocTypes = [];
        ...
        if (!is_array($UploadedDocs) && !is_object($UploadedDocs))
            $UploadedDocs = [];
    ...
    public function PublishProperty($PropertyId = 0)
    {
        ...
        $missingGeneral = [];
        if (empty(trim($prop->PropertyTitle ?? '')))
            $missingGeneral[] = "Property Title";
        if (empty(trim($prop->CoveredArea ?? '')))
            $missingGeneral[] = "Covered Area";
        if (empty(trim($prop->PropertyStatus ?? '')))
            $missingGeneral[] = "Property Status";
        if (empty(trim($prop->PropertyDescription ?? '')))
            $missingGeneral[] = "Description";
        if (empty(trim($prop->MailingAddress ?? '')))
            $missingGeneral[] = "Mailing Address";
        ...
        foreach ($DocTypes as $type) {
            if ($type->IsMandatory == 1) {
                ...
                if (!$found)
                    $missingDocs[] = $type->DocumentTitle;
            }
        }
