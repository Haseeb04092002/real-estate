...
// Format address
$addrParts = [];
if (!empty($value->UnitNumber))
    $addrParts[] = $value->UnitNumber;
if (!empty($value->StreetNumber))
    $addrParts[] = $value->StreetNumber;
if (!empty($value->StreetName))
    $addrParts[] = $value->StreetName;
if (!empty($value->Suburb))
    $addrParts[] = $value->Suburb;
if (!empty($value->State))
    $addrParts[] = strtoupper($value->State);
...
<div class="<?= $GridClass ?> wow fadeInUp property-item-box <?= htmlspecialchars($ListType); ?>" data-wow-delay="0.1s"
    data-search="<?= strtolower(
        ...
        ($value->Suburb ?? '') . ' ' .
        ($value->Postcode ?? '')
    ); ?>" data-price="<?= $TotalPrice; ?>" data-bedrooms="<?= is_numeric($Bedrooms) ? $Bedrooms : 0; ?>"
    data-bathrooms="<?= is_numeric($Bathrooms) ? $Bathrooms : 0; ?>" data-type="<?= $PropertyTypeId; ?>"
    data-state="<?= strtolower($value->State ?? ''); ?>" data-suburb="<?= strtolower($value->Suburb ?? ''); ?>"
    data-coveredarea="<?= $CoveredArea; ?>" data-listtype="<?= strtolower($ListType); ?>"
    ...
    <div class="prop-card" onclick="window.location='<?= site_url('Properties/PropertyDetails/' . $PropertyId); ?>'">
        ...
        <div class="prop-card-img-wrap">
            ...
            <div class="prop-badge-top-right d-flex flex-row align-items-end">
                ...
                <?php if (isset($ShowStatusBadge) && $ShowStatusBadge): ?>
                    ...
                    <?php else: ?>
                        <span style="background-color: #ffc107; color: #000;"><i class="fa fa-clock me-1"></i> Not
                            Published</span>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        ...
        <div class="prop-card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="prop-title" style="max-width: 65%;" title="<?= $PropertyTitle; ?>"><?= $PropertyTitle; ?>
                </div>
                <div class="prop-price">
                    $<?= number_format($TotalPrice); ?><?= (strtolower($ListType) == 'rent') ? '/week' : '' ?></div>
            </div>

            <div class="prop-address mb-2 rounded p-2 text-dark"
                style="background-color: #f2f2f2; font-size: 0.9rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                title="<?= htmlspecialchars($displayAddress); ?>">
            ...
            <?php if (isset($DashboardLayout) && $DashboardLayout): ?>
                <div class="d-flex flex-column mt-auto mb-3">
                    <div class="prop-features mb-3 mt-0">
                        <div><i class="fa fa-bed"></i> <?= is_numeric($Bedrooms) && $Bedrooms > 0 ? $Bedrooms : '0'; ?>
                        </div>
                        <div><i class="fa fa-bath"></i> <?= is_numeric($Bathrooms) && $Bathrooms > 0 ? $Bathrooms : '0'; ?>
                        </div>
                    ...
                    <div class="d-flex gap-2 justify-content-start flex-wrap">
                        <?php if (isset($UserId) && isset($value->AddedBy) && $UserId == $value->AddedBy): ?>
                            <a href="<?= site_url('Properties/AddListing/' . $PropertyId . '/Edit'); ?>"
                                class="btn btn-outline-primary btn-sm px-4 rounded-pill" style="font-weight: 500;"
                                onclick="event.stopPropagation();">Edit</a>
                            <button type="button" class="btn btn-outline-secondary btn-sm px-4 rounded-pill"
                                style="font-weight: 500;" data-bs-toggle="modal"
                                data-bs-target="#statusModal_<?= $PropertyId ?>" onclick="event.stopPropagation();">Change
                                Status</button>
                        <?php endif; ?>
                ...
                <?php if (isset($UserId) && isset($value->AddedBy) && $UserId == $value->AddedBy): ?>
                    <!-- Status Modal -->
                    <div class="modal fade" id="statusModal_<?= $PropertyId ?>" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
                                ...
                                <div class="modal-body p-4">
                                    <form id="statusForm_<?= $PropertyId ?>">
                                        ...
                                        <div class="mb-4 d-flex align-items-start">
                                            <input type="radio" name="Status_<?= $PropertyId ?>"
                                                id="status_pub_<?= $PropertyId ?>" value="Published" <?= (isset($value->Status) && strtolower($value->Status) == 'published') ? 'checked' : '' ?>
                                                style="margin-top: 4px; margin-right: 12px; transform: scale(1.2); appearance: radio !important; opacity: 1 !important; position: static !important; cursor: pointer; display: block !important; visibility: visible !important;">
                                            <div>
                                                <label class="fw-bold text-success mb-1" for="status_pub_<?= $PropertyId ?>"
                                                    style="cursor: pointer;">
                                                    <i class="fa fa-check-circle me-1"></i> Published
                                                </label>
                                                <div class="text-muted small">Visible to everyone in public search results.
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mb-4 d-flex align-items-start">
                                            <input type="radio" name="Status_<?= $PropertyId ?>"
                                                id="status_unpub_<?= $PropertyId ?>" value="Not Published"
                                                <?= (!isset($value->Status) || empty(trim($value->Status)) || in_array(strtolower($value->Status), ['not published', 'draft', 'pending'])) ? 'checked' : '' ?>
                                                style="margin-top: 4px; margin-right: 12px; transform: scale(1.2); appearance: radio !important; opacity: 1 !important; position: static !important; cursor: pointer; display: block !important; visibility: visible !important;">
                                            <div>
                                                <label class="fw-bold text-warning mb-1"
                                                    style="color: #d39e00 !important; cursor: pointer;"
                                                    for="status_unpub_<?= $PropertyId ?>">
                                                    <i class="fa fa-clock me-1"></i> Not Published
                                                </label>
                                                <div class="text-muted small">Hidden from public listings, visible only to you.
                                                </div>
                                            </div>
                                        </div>
                                    </form>
                                ...
                                <div class="modal-footer border-top-0 bg-light rounded-bottom">
                                    ...
                                    <button type="button" class="btn btn-primary px-4 rounded-pill fw-bold"
                                        onclick="savePropertyStatus(<?= $PropertyId ?>)">Save Status</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="prop-agent">
                ...
                <div class="d-flex flex-column justify-content-center">
                    <span class="prop-agent-name d-flex align-items-center">
                        <?= $agentName; ?>
                        <?php if ($isVerified): ?>
                            <span style="border: 1px solid #1b9500c5; background-color: rgba(32, 176, 0, 0.2);"
                                class="badge ms-2 rounded-pill text-dark">Verified</span>
                        <?php else: ?>
                            <span style="border: 1px solid #95840066; background-color: rgba(255, 191, 0, 0.3);"
...
<?php if (!defined('PROP_CARD_STATUS_SCRIPT')):
    define('PROP_CARD_STATUS_SCRIPT', 1); ?>
    <script>
