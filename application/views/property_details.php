...
foreach ($mappings as $m) {
    if (strtolower($m->Title) == 'garages')
        $PropertyFeatures->Garages = $m->FeatureValue;
    else if (strtolower($m->Title) == 'garage size sqft')
        $PropertyFeatures->GarageSize = $m->FeatureValue;
    else {
        ...
        if (in_array(strtolower($m->Title), $structTitles)) {
            ...
            if (strtolower($m->Title) == 'bedrooms' && empty($PropertyFeatures->Bedrooms))
                $PropertyFeatures->Bedrooms = $m->FeatureValue;
            if (strtolower($m->Title) == 'bathrooms' && empty($PropertyFeatures->Bathrooms))
                $PropertyFeatures->Bathrooms = $m->FeatureValue;
            if ((strtolower($m->Title) == 'built in year' || strtolower($m->Title) == 'year built') && empty($PropertyFeatures->BuiltInYear))
                $PropertyFeatures->BuiltInYear = $m->FeatureValue;
        } else {
...
<?php if (!isset($IsPreview) || !$IsPreview): ?>
    ...
    <body>
        ...
        <div class="container-lg mt-4 mb-5">
            ...
            <?php if (isset($IsPreview) && $IsPreview): ?>
                <div
                    class="alert alert-info d-flex flex-column flex-md-row justify-content-between align-items-center shadow-sm mb-4">
                    <div class="fw-bold mb-3 mb-md-0"><i class="fa fa-eye me-2"></i> This is a preview of how your property
                        will look once published.</div>
                    <button type="button" class="btn btn-success fw-bold px-4 shadow-sm"
                        onclick="publishProperty(<?= $PropertyDetails->PropertyId ?>)"><i
                            class="fa-solid fa-paper-plane me-2"></i>Publish Property</button>
                </div>
                        ...
                        <div class="modal-content border-0 shadow">
                            <div class="modal-header bg-danger text-white">
                                <h5 class="modal-title text-white"><i
                                        class="text-white fa-solid fa-triangle-exclamation me-2"></i> Incomplete Details
                                </h5>
                            ...
                            <div class="modal-footer border-0 justify-content-center pb-4">
                                <button type="button" class="btn btn-secondary px-4 fw-bold" data-bs-dismiss="modal">OK, I
                                    will fill them</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="text-muted small">
                        ...
                        if (!empty($PropertyDetails->MailingAddress)) {
                            ...
                            if (!empty($PropertyDetails->UnitNumber))
                                $dispAddr[] = "Unit " . $PropertyDetails->UnitNumber;
                            ...
                            if (!empty($PropertyDetails->Suburb))
                                $dispAddr[] = $PropertyDetails->Suburb;
                            if (!empty($PropertyDetails->State))
                                $dispAddr[] = $PropertyDetails->State;
                            if (!empty($PropertyDetails->ZipCode))
                                $dispAddr[] = $PropertyDetails->ZipCode;
                        }
                ...
                <div class="text-md-end">
                    <?php if (($PropertyDetails->ListType ?? 'Sale') === 'Rent'): ?>
                        ...
                        <div class="text-primary-custom fw-bold" style="font-size: 34px; line-height: 1;">
                            $<?= number_format($PropertyDetails->TotalPrice ?? 0); ?> <span style="font-size: 18px;"
                                class="text-muted fw-normal">/Month</span>
                        </div>
                    <?php else: ?>
                        <?php if (!empty($PropertyDetails->CoveredArea) && $PropertyDetails->CoveredArea > 0): ?>
                            <div class="text-primary-custom fw-bold" style="font-size: 15px;">
                                $<?= number_format(($PropertyDetails->TotalPrice ?? 0) / $PropertyDetails->CoveredArea, 2); ?>
                                <span class="text-muted fw-normal">/<?= $AreaUnit ?></span>
                            </div>
                        <?php endif; ?>
                        <div class="text-primary-custom fw-bold" style="font-size: 34px; line-height: 1;">
                    ...
                    <div class="mt-3 d-flex justify-content-md-end gap-2">
                        <button class="action-btn" onclick="copyToClipboard(window.location.href)"><i
                                class="fa-solid fa-share-nodes"></i> Share</button>

                        <?php if ($IsFavourite): ?>
                            <button class="action-btn text-danger border-danger"
                                onclick="window.location.href='<?= base_url('Properties/RemoveFromFavourites/' . $PropertyId) ?>'"><i
                                    class="fa-solid fa-heart"></i> Favorite</button>
                        <?php else: ?>
                            <button class="action-btn"
                                onclick="window.location.href='<?= base_url('Properties/AddToFavourites/' . $PropertyId) ?>'"><i
                                    class="fa-regular fa-heart"></i> Favorite</button>
                        <?php endif; ?>

                        <button class="action-btn" onclick="window.print()"><i class="fa-solid fa-print"></i>
                            Print</button>
                    </div>
                    <script>
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                        ...
                        <div class="swiper mySwiper2" style="height: 500px; border-radius: 4px;">
                            <div class="swiper-wrapper">
                                <?php foreach ($PropertyImages as $value): ?>
                                    <div class="swiper-slide"><img src="<?= $value->path ?>" class="w-100 h-100"
                                            style="object-fit: cover;" alt="Image"></div>
                                <?php endforeach; ?>
                            </div>
                            <div class="swiper-button-next" style="color: #fff; text-shadow: 0 0 5px rgba(0,0,0,0.5);">
                            </div>
                            <div class="swiper-button-prev" style="color: #fff; text-shadow: 0 0 5px rgba(0,0,0,0.5);">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="swiper mySwiper mb-4" style="height: 90px; cursor: pointer;">
                        <div class="swiper-wrapper">
                            <?php foreach ($PropertyImages as $value): ?>
                                <div class="swiper-slide"><img src="<?= $value->path ?>" class="w-100 h-100 rounded-1"
                                        style="object-fit: cover;" alt="Thumb"></div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                        ...
                        <div class="row text-center g-3">
                            <div class="col">
                                ...
                                <div class="small fw-bold text-dark">
                                    <?= date("F j, Y", strtotime($PropertyDetails->CreatedAt ?? date("Y-m-d"))) ?>
                                </div>
                            </div>
                            <div class="col">
                                <i class="fa-solid fa-bed fs-3 overview-icon"></i>
                                <div class="fw-bold small text-dark"><?= $PropertyFeatures->Bedrooms ?? "0" ?> Bedrooms
                                </div>
                            </div>
                            <div class="col">
                                <i class="fa-solid fa-bath fs-3 overview-icon"></i>
                                <div class="fw-bold small text-dark"><?= $PropertyFeatures->Bathrooms ?? "0" ?>
                                    Bathrooms</div>
                            </div>
                            <div class="col">
                                <i class="fa-solid fa-car fs-3 overview-icon"></i>
                                <div class="fw-bold small text-dark"><?= $PropertyFeatures->Garages ?? "0" ?> Garages
                                </div>
                            </div>
                            <div class="col">
                                <i class="fa-solid fa-vector-square fs-3 overview-icon"></i>
                                <div class="fw-bold small text-dark"><?= $PropertyDetails->CoveredArea ?? "" ?>
                                    <?= $AreaUnit ?>
                                </div>
                            </div>
                            <div class="col">
                                <i class="fa-regular fa-calendar-days fs-3 overview-icon"></i>
                                <div class="fw-bold small text-dark">Year Built:
                                    <?= !empty($PropertyFeatures->BuiltInYear) ? date("Y", strtotime($PropertyFeatures->BuiltInYear)) : "N/A" ?>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-6">
                                <ul class="detail-list">
                                    ...
                                    <li><strong>Price:</strong>
                                        <span>$<?= number_format($PropertyDetails->TotalPrice ?? 0); ?></span>
                                    </li>
                                    <li><strong>Property Size:</strong> <span><?= $PropertyDetails->CoveredArea ?? "" ?>
                                            <?= $AreaUnit ?></span></li>
                                    <li><strong>Bedrooms:</strong>
                                        <span><?= $PropertyFeatures->Bedrooms ?? "0" ?></span>
                                    </li>
                                    <li><strong>Bathrooms:</strong>
                                        <span><?= $PropertyFeatures->Bathrooms ?? "0" ?></span>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-md-6">
                                <ul class="detail-list">
                                    <li><strong>Garage:</strong> <span><?= $PropertyFeatures->Garages ?? "0" ?></span>
                                    </li>
                                    <li><strong>Garage Size:</strong> <span><?= $PropertyFeatures->GarageSize ?? "0" ?>
                                            <?= $AreaUnit ?></span></li>
                                    <li><strong>Year Built:</strong>
                                        <span><?= !empty($PropertyFeatures->BuiltInYear) ? date("Y", strtotime($PropertyFeatures->BuiltInYear)) : "N/A" ?></span>
                                    </li>
                                    <li><strong>Property Type:</strong> <span><?= $PropertyType ?? "" ?></span></li>
                                    <li><strong>Property Status:</strong> <span>For
                                            <?= ucfirst(strtolower($PropertyDetails->ListType ?? "")) ?></span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion mb-4 shadow-sm border rounded-2" id="accordionPropertyAddress">
                        <div class="accordion-item border-0">
                            <h2 class="accordion-header" id="heading_accordion_property_address_collapse">
                                <button class="accordion-button fw-bold bg-white text-dark rounded-top-2" type="button"
                                    data-bs-toggle="collapse" data-bs-target="#accordion_property_address_collapse"
                            ...
                            <div id="accordion_property_address_collapse" class="accordion-collapse collapse show"
                                ...
                                <div class="accordion-body text-muted bg-white rounded-bottom-2 border-top">
                                    <div class="row g-3">
                                        <div class="col-md-12 mb-2 border-bottom pb-2"><strong>Mailing Address:</strong>
                                            <span class="text-dark"><?= $PropertyDetails->MailingAddress ?? "" ?></span>
                                        </div>
                                        <div class="col-md-4"><strong>Country:</strong>
                                            <?= $PropertyDetails->Country ?? "Australia" ?></div>
                                        <div class="col-md-4"><strong>State:</strong>
                                            <?= $PropertyDetails->State ?? "" ?></div>
                                        <div class="col-md-4"><strong>Suburb:</strong>
                                            <?= $PropertyDetails->Suburb ?? "" ?></div>
                                        <div class="col-md-4"><strong>Postal Code:</strong>
                                            <?= $PropertyDetails->ZipCode ?? "" ?></div>
                                        <div class="col-md-4"><strong>Unit Number:</strong>
                                            <?= $PropertyDetails->UnitNumber ?? "" ?></div>
                                        <div class="col-md-4"><strong>Street Number:</strong>
                                            <?= $PropertyDetails->StreetNumber ?? "" ?></div>
                                        <div class="col-md-4"><strong>Street Name:</strong>
                                            <?= $PropertyDetails->StreetName ?? "" ?></div>
                    ...
                    <div class="accordion mb-4 shadow-sm border rounded-2" id="accordionTravelTime">
                        <div class="accordion-item border-0">
                            <h2 class="accordion-header" id="headingTravelTime">
                                <button class="accordion-button fw-bold bg-white text-dark rounded-top-2" type="button"
                                    data-bs-toggle="collapse" data-bs-target="#collapseTravelTime" aria-expanded="true"
                                    aria-controls="collapseTravelTime">
                                    <div>
                            ...
                            <div id="collapseTravelTime" class="accordion-collapse collapse show"
                                aria-labelledby="headingTravelTime" data-bs-parent="#accordionTravelTime">
                                    ...
                                    <form class="p-1 pt-0 mt-0">
                                        ...
                                        <div class="row g-3 mt-2">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" id="txtSearchCurrLocation"
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control bg-light border-0 text-muted"
                                                    value="<?= $PropertyDetails->MailingAddress ?? "" ?>" readonly="">
                                            </div>
                                        </div>
                                        ...
                                        <div class="row g-3 mt-2 mb-3">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" id="distance"
                                                    class="form-control p-2 bg-light border-0 text-primary-custom fw-bold"
                                                    readonly="">
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label fw-bold text-dark fs-6">Estimated Time</label>
                                                <input type="text"
                                                    class="form-control p-2 bg-light border-0 text-success fw-bold"
                                                    id="travelTime" readonly="">
                                            </div>
                                        </div>
                                    </form>
                        ...
                        <div class="features-grid text-muted" style="font-size: 15px;">
                            ...
                            <?php if ($hasAmenities): ?>
                                <?php foreach ($DynamicFeaturesList as $name => $val): ?>
                                    <?php if ($val === true): ?>
                                        <div class="feature-item"><i class="far fa-check-circle"></i> <?= htmlspecialchars($name) ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="feature-item"><i class="far fa-check-circle"></i>
                                            <?= htmlspecialchars($name) ?>: <strong
                                                class="text-dark"><?= htmlspecialchars($val) ?></strong></div>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                            <?php else: ?>
                                        ...
                                        <div class="d-flex align-items-center p-3 border rounded-2 bg-light shadow-sm">
                                            ...
                                            <div class="flex-grow-1 overflow-hidden">
                                                <a href="<?= $docPath ?>" target="_blank"
                                                    class="text-dark text-decoration-none fw-bold d-block text-truncate"
                                                    title="<?= htmlspecialchars($doc->FilePath ?? '') ?>">
                                            ...
                                            <a href="<?= $docPath ?>" download class="btn btn-sm btn-outline-primary ms-2"><i
                    ...
                    <div class="bg-white p-4 rounded-2 shadow-sm border mb-4">
                        ...
                        <div class="mb-4">
                            <div class="d-flex mb-4">
                                <img src="https://ui-avatars.com/api/?name=Sarah+M&background=random"
                                    class="rounded-circle me-3" style="width: 50px; height: 50px;" alt="Reviewer">
                                <div>
                                    ...
                                    <p class="text-muted small mb-0">We loved the property. The house is beautiful and
                                        situated in a peaceful neighborhood. Highly recommend it to anyone looking for a
                                        quiet retreat.</p>
                                </div>
                            </div>
                            <hr class="text-muted opacity-25">
                            <div class="d-flex mb-2">
                                <img src="https://ui-avatars.com/api/?name=David+R&background=random"
                                    class="rounded-circle me-3" style="width: 50px; height: 50px;" alt="Reviewer">
                                <div>
                                    ...
                                    <p class="text-muted small mb-0">Overall a fantastic experience. The backyard is
                                        enormous and perfect for barbecues. A few minor things inside could use some
                                        updating, but the price reflects the value.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Add Review Form -->
                        <div <?= isset($UserId) && !empty($UserId) ? 'class= d-block' : 'class= d-none' ?>>
                            ...
                            <div class="add_review_wrapper">
                                ...
                                <form>
                                    <input type="text" id="wpestate_review_title" name="wpestate_review_title"
                                        class="form-control mb-3 bg-light border-0 py-2 rounded-1"
                                        placeholder="Review Title">
                                    <textarea rows="5" id="wpestare_review_content" name="wpestare_review_content"
                                        class="form-control mb-3 bg-light border-0 py-2 rounded-1"
                                        placeholder="Your Review"></textarea>
                                    <button type="button"
                                        class="btn btn-primary-custom py-2 px-4 rounded-1 fw-bold border-0 shadow-sm"
                                        id="submit_review">Submit Review</button>
                                </form>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Sidebar Column -->
                <div class="col-lg-4">
                    <div class="bg-white rounded-2 shadow-sm border overflow-hidden position-sticky"
                        style="top: 20px; z-index: 10;">

                        <!-- Tabs -->
                        <ul class="nav nav-tabs nav-fill" id="sidebarTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active fw-bold py-3 rounded-0 px-2" id="request-info-tab"
                                    data-bs-toggle="tab" data-bs-target="#request-info" type="button" role="tab"
                                    aria-controls="request-info" aria-selected="true" style="font-size:13px;">Request
                                    Info</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold py-3 rounded-0 px-2" id="schedule-tour-tab"
                                    data-bs-toggle="tab" data-bs-target="#schedule-tour" type="button" role="tab"
                                    aria-controls="schedule-tour" aria-selected="false" style="font-size:13px;">Schedule
                                    a tour</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link fw-bold py-3 rounded-0 px-2" id="make-offer-tab"
                                    data-bs-toggle="tab" data-bs-target="#make-offer" type="button" role="tab"
                                    aria-controls="make-offer" aria-selected="false" style="font-size:13px;">Make
                                    Offer</button>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content" id="sidebarTabsContent">
                            ...
                            <!-- Request Info Tab -->
                            <div class="tab-pane fade show active p-4" id="request-info" role="tabpanel"
                                aria-labelledby="request-info-tab">
                                <!-- Agent Info -->
                                <div class="d-flex align-items-center mb-4">
                                    <div class="rounded-circle me-3 d-flex align-items-center justify-content-center text-white fw-bold bg-primary-custom shadow-sm"
                                        style="width: 65px; height: 65px; font-size: 24px;">
                                        <?= htmlspecialchars($SellerInitials ?? 'SA') ?>
                                    </div>
                                    <div>
                                        <h5 class="mb-1 fw-bold text-dark d-flex align-items-center">
                                            ...
                                            <?php if (($Seller->AccountStatus ?? '') === 'Active' || ($Seller->AccountStatus ?? '') === 'Verified'): ?>
                                                <span
                                                    style="border: 1px solid #1b9500c5; background-color: rgba(32, 176, 0, 0.2);"
                                                    class="badge ms-2 rounded-pill text-dark">Verified</span>
                                            <?php else: ?>
                                                <span
                                                    style="border: 1px solid #95840066; background-color: rgba(255, 191, 0, 0.3);"
                                                    class="badge text-dark ms-2 rounded-pill">Not Verified</span>
                                            <?php endif; ?>
                                        </h5>
                                        <div class="text-muted small">Member ID:
                                            <?= htmlspecialchars($Seller->ClientId ?? '0') ?>
                                        </div>
                                    </div>
                                </div>

                                <!-- Schedule Banner -->
                                <div class="bg-success text-white text-center py-2 rounded-1 mb-4 fw-semibold shadow-sm"
                                ...
                                <form id="frmRequestInfo">
                                    ...
                                    <input type="text" id="req_name"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Name"
                                        value="<?= $autoName ?>" <?= $autoReadonly ?> required>
                                    <input type="email" id="req_email"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Email"
                                        value="<?= $autoEmail ?>" <?= $autoReadonly ?> required>
                                    <input type="tel" id="req_phone"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Phone"
                                        value="<?= $autoPhone ?>" <?= $autoReadonly ?> required>

                                    <select id="req_action"
                                        class="form-select mb-3 py-2 rounded-1 text-muted bg-light border-0">
                                        <option value="">What are you looking to do?</option>
                                        <option value="I want to schedule a viewing">I want to schedule a viewing
                                        </option>
                                        <option value="I want more details">I want more details</option>
                                    </select>

                                    <textarea id="req_message"
                                        class="form-control mb-4 py-2 rounded-1 text-muted bg-light border-0" rows="3">Hello,
I'm interested in [ <?= $PropertyDetails->PropertyTitle ?? "" ?> ]</textarea>

                                    <!-- Buttons -->
                                    <div class="d-flex gap-2">
                                        <button type="button" id="btnSendRequestInfo"
                                            class="btn btn-primary-custom flex-fill py-2 rounded-2"
                                            style="font-size: 14px; border: 1px solid #0066ff;">
                                            <i class="fa-solid fa-paper-plane me-1"></i> Send Request
                                        </button>
                                        <a href="tel:<?= htmlspecialchars($Seller->PhoneNumber ?? '') ?>"
                                            class="btn btn-outline-primary-custom flex-fill py-2 rounded-2 d-flex align-items-center justify-content-center text-decoration-none"
                                            style="font-size: 14px; border: 1px solid #0066ff;">
                                            <i class="fa-solid fa-phone me-1"></i> Call
                                        </a>
                                        <a href="https://wa.me/<?= preg_replace('/[^0-9]/', '', $Seller->PhoneNumber ?? '') ?>"
                                            target="_blank"
                                            class="btn btn-outline-primary-custom flex-fill py-2 rounded-2 d-flex align-items-center justify-content-center text-decoration-none"
                                            style="font-size: 14px; border: 1px solid #0066ff;">
                            ...
                            <!-- Schedule a tour Tab -->
                            <div class="tab-pane fade p-4" id="schedule-tour" role="tabpanel"
                                aria-labelledby="schedule-tour-tab">
                                <h5 class="fw-bold text-dark mb-4">Schedule a tour</h5>

                                <!-- Dates -->
                                <div class="row g-2 mb-4">
                                    <div class="col-6">
                                        <input type="text" id="tour_date"
                                            class="form-control py-2 rounded-1 text-muted bg-light border-0 shadow-sm"
                                            placeholder="Select Date" required>
                                    </div>
                                    <div class="col-6">
                                        <input type="text" id="tour_time"
                                            class="form-control py-2 rounded-1 text-muted bg-light border-0 shadow-sm"
                                ...
                                <div class="d-flex bg-light p-1 rounded-2 mb-4 border">
                                    <input type="radio" class="btn-check" name="tour_type" id="tour_in_person"
                                        autocomplete="off" checked>
                                    <label
                                        class="btn flex-fill rounded-2 border-0 fw-bold py-2 tour-type-btn text-muted m-0"
                                        for="tour_in_person"><i class="fa-regular fa-user me-2"></i> In Person</label>

                                    <input type="radio" class="btn-check" name="tour_type" id="tour_video"
                                        autocomplete="off">
                                    <label
                                        class="btn flex-fill rounded-2 border-0 fw-bold py-2 tour-type-btn text-muted m-0"
                                        for="tour_video"><i class="fa-solid fa-video me-2"></i> Video Chat</label>
                                </div>
                                ...
                                <form>
                                    <input type="text" id="tour_name"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Name"
                                        value="<?= $autoName ?>" <?= $autoReadonly ?>>
                                    <input type="email" id="tour_email"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Email"
                                        value="<?= $autoEmail ?>" <?= $autoReadonly ?>>
                                    <input type="tel" id="tour_phone"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Phone"
                                        value="<?= $autoPhone ?>" <?= $autoReadonly ?>>

                                    <select class="form-select mb-3 py-2 rounded-1 text-muted bg-light border-0">
                                        <option>What are you looking to do?</option>
                                    </select>

                                    <textarea id="tour_message"
                                        class="form-control mb-4 py-2 rounded-1 text-muted bg-light border-0"
                                        rows="4">I would like to schedule a tour for [ <?= $PropertyDetails->PropertyTitle ?? "" ?> ].</textarea>

                                    <button type="button" id="btnScheduleTour"
                                        class="btn btn-primary-custom w-100 py-2 rounded-2 mt-2"
                                        style="font-size: 14px; border: 1px solid #0066ff;"><i
                                            class="fa-solid fa-paper-plane me-1"></i> Send Request</button>
                                </form>
                            </div>

                            <!-- Make Offer Tab -->
                            <div class="tab-pane fade p-4" id="make-offer" role="tabpanel"
                                aria-labelledby="make-offer-tab">
                                ...
                                <form id="frmMakeOffer">
                                    ...
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="number" id="offer_amount"
                                            class="form-control py-2 rounded-end-1 bg-light border-0"
                                    ...
                                    <input type="text" id="offer_name"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Name"
                                        value="<?= $autoName ?>" <?= $autoReadonly ?> required>
                                    <input type="email" id="offer_email"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Email"
                                        value="<?= $autoEmail ?>" <?= $autoReadonly ?> required>
                                    <input type="tel" id="offer_phone"
                                        class="form-control mb-3 py-2 rounded-1 bg-light border-0" placeholder="Phone"
                                        value="<?= $autoPhone ?>" <?= $autoReadonly ?> required>

                                    <textarea id="offer_message"
                                        class="form-control mb-4 py-2 rounded-1 text-muted bg-light border-0" rows="3"
                                        placeholder="Additional details or terms (optional)"></textarea>

                                    <button type="button" id="btnMakeOffer"
                                        class="btn btn-primary-custom w-100 py-2 rounded-2 mt-2"
                                        style="font-size: 14px; border: 1px solid #0066ff;"><i
                                            class="fa-solid fa-paper-plane me-1"></i> Submit Offer</button>
                                </form>
                            </div>
                        </div>
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCv1FrfWK8d_Z28pT_XtiZW02msCfrC2Rs&amp;libraries=places,geometry&amp;callback=initMap"
        async="" defer=""></script>
    <?php if (!isset($IsPreview) || !$IsPreview): ?>
